...
public class RuleBasedProductMatchScorer {

	private final ProductNameNormalizer normalizer;

	public ProductMatchResult score(Product product, KcaProductPriceItemResponse externalProduct) {
		String internalName = normalizer.normalize(product.getProductName());
		String externalName = normalizer.normalize(externalProduct.goodName());

		double score = calculateNameScore(internalName, externalName);
		score += calculateBrandBonus(product.getBrandName(), externalProduct.productEntpName());
		score += calculateVolumeBonus(product.getVolume(), externalProduct.goodName());
