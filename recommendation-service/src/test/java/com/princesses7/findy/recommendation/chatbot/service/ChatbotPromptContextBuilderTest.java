...
class ChatbotPromptContextBuilderTest {
	...
	@Test
	@DisplayName("시스템 프롬프트, 쇼핑 데이터, 최근 대화, 현재 메시지를 순서대로 조합한다")
	void buildPromptWithShoppingContextAndRecentContext() {
		...
		assertThat(messages.get(1).content()).contains("사리곰탕 재고 3개");
	}
}